...
describe("Interface Resolution", () => {
    ...
    it("should allow swapping implementations", () => {
        ...
        consoleBuilder.register((b) =>
            b.bind(ILoggerToken).to(ConsoleLogger).asSingleton()
        );
        const consoleContainer = consoleBuilder.build();

        const fileBuilder = new ContainerBuilder();
        fileBuilder.register((b) =>
            b.bind(ILoggerToken).to(FileLogger).asSingleton()
        );
    ...
    it("should inject interface dependencies into services", () => {
        ...
        builder.register((b) =>
            b.bind(IConfigToken).to(InMemoryConfig).asSingleton()
        );
        ...
        // Configure config on the built container
    ...
    it("should support multiple interface implementations with different tokens", () => {
        ...
        builder.register((b) =>
            b.bind(ConsoleLoggerToken).to(ConsoleLogger).asSingleton()
        );
        builder.register((b) =>
            b.bind(FileLoggerToken).to(FileLogger).asSingleton()
        );
    ...
    it("should support string tokens for interfaces", () => {
        ...
        builder.register((b) =>
            b.bind(LoggerServiceToken).to(ConsoleLogger).asSingleton()
        );
        builder.register((b) =>
            b.bind(ConfigServiceToken).to(InMemoryConfig).asSingleton()
        );
